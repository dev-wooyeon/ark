...
이 글에서는 Next.js 16, Motion, MDX를 선택한 이유,
아키텍처 원칙,
최적화 기준,
...
<strong>[zerolog.vercel.app](https://zerolog.vercel.app)</strong>

미니멀하면서도 완성도 높은 디자인, 깔끔한 타이포그래피, 그리고 세심한 인터랙션.
단순히 "이쁘다"를 넘어 <strong>"어떻게 구현했을까?"</strong> 를 고민하게 만드는 블로그였다.
...
오히려 <strong>"왜 이렇게 만들었을까?"</strong> 를 고민하며 스스로 구현해보는 과정이 가장 좋은 학습이었다.
...
### 2.4 Interactive: Framer Motion

**선택 이유:**

- 테마 전환과 목록 진입처럼 상태 변화를 자연스럽게 전달
- `prefers-reduced-motion`을 반영한 접근성 있는 모션 제어
- React 생태계와 자연스러운 통합
...
- 브라우저 API를 쓰는 leaf component에만 `'use client'`를 둔다
- transform과 opacity 중심으로 움직여 레이아웃 비용을 피한다
- reduced motion에서는 즉시 또는 짧은 전환으로 낮춘다
...
<strong>"최신 기술은 생산성을 위해, 안정성도 함께 고려."</strong>

- Next.js 16: App Router의 안정성 확보
- Tailwind v4: 빌드 속도 개선 체감
- MDX: React 통합으로 확장성 극대화

<strong>"번들 사이즈는 항상 모니터링."</strong>

무거운 클라이언트 라이브러리는 실제 공개 기능에 필요할 때만 추가한다.
동적 import와 `'use client'` 경계로 초기 로딩 경로를 작게 유지한다.

### 8.3 최적화

<strong>"측정하지 않으면 최적화할 수 없다."</strong>

Lighthouse, Bundle Analyzer를 통해 정량적 지표를 확보했다.
추측이 아닌 데이터 기반 최적화다.

<strong>"점진적 개선이 급진적 리팩토링보다 안전하다."</strong>
